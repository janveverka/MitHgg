// $Id: HGGModSimple.cc,v 1.6 2010/03/12 13:51:26 bendavid Exp $
...
void HGGModSimple::FillDiphotonTree(const Photon *g1, const Photon *g2) const {
  ...
  //  if (!geng1 && !geng2) printf("Double fake!!!\n");
    ...
    Double_t rho = -99.;
    if (fPileUpDen->GetEntries()) rho = fPileUpDen->At(0)->Rho();
    ...
    if (bestconv) {
      ...
      if (bestconv==conv1) gbestconv = g1;
      else if (bestconv==conv2) gbestconv = g2;
      
      Bool_t usetrackz = (gbestconv->IsEB() && bestconv->Position().Rho()<15.0) || (gbestconv->IsEE() && TMath::Abs(bestconv->Position().Z())<100.0);
      ...
      for (UInt_t ivtx=0; ivtx<fPrimaryVertex->GetEntries(); ++ivtx) {
        ...
        Double_t dz;
        if (usetrackz) dz = TMath::Abs(bestconv->DzCorrected(thevtx));
        else dz = TMath::Abs(bestconv->Z0EcalVtx(thevtx->Position(),ThreeVector(gbestconv->SCluster()->Point())) - thevtx->Z());
        if ( dz < mindz ) {
          mindz = dz;
