...
void HGGMod::Process()
{
  ...
  if ( (!fSinglePhotonMode && fPhotons->GetEntries()<2) ) return;
  ...
  //const VertexCol *goodVertexes = GetObjThisEvt<VertexCol>("GoodVertexes");
...
void HGGMod::FillDiphotonTree(const Photon *g1, const Photon *g2) const {
  ...
  //  if (!geng1 && !geng2) printf("Double fake!!!\n");
    ...
    if (!vtxmulti) vtxmulti = fPrimaryVertex->At(0);
...
const DecayParticle *HGGMod::MatchConversion(const Photon *p) const
{
  ...
  Double_t minrho = 999.;
  //printf("Photon pt = %5f, eta = %5f, phi = %5f\n",p->Pt(),p->Eta(),p->Phi());
...
//    if (c->Quality().Quality(ConversionQuality::arbitratedMerged) && c->Prob()>0. && MathUtils::DeltaR(p,c)<0.3 ) {
   // if (c->Quality().Quality(ConversionQuality::arbitratedEcalSeeded) && c->Prob()>0. && MathUtils::DeltaR(p,c)<0.3 ) {
    if ( c->Prob()>1e-6 && c->LxyCorrected(fBeamSpot->At(0))>2.0 ) {  
      ...
      Int_t algo1 = dynamic_cast<const StableParticle*>(c->Daughter(0))->Trk()->Algo();
      Int_t algo2 = dynamic_cast<const StableParticle*>(c->Daughter(1))->Trk()->Algo();
    ...
    //  printf("Conversion pt = %5f, eta = %5f, phi = %5f, eop = %5f, rho = %5f, z = %5f, algo1 = %i, algo2 = %i, prob = %5f, nhb1 = %i, nhb2 = %i\n",c->Pt(),c->Eta(),c->Phi(),p->SCluster()->Energy()/c->P(),c->Position().Rho(),c->Position().Z(),algo1,algo2,c->Prob(),nhb1,nhb2);
      //if (drconv<0.1 && TMath::Max(nhb1,nhb2)<2 && c->Position().Rho()<minrho) {
      if ( drconv<0.1 && TMath::Max(nhb1,nhb2)<2 && c->Position().Rho()<minrho) {
      //if ( (algo1==15||algo1==16) && (algo2==15||algo2==16) && MathUtils::DeltaR(*p,*c)<0.3 && TMath::Max(nhb1,nhb2)<2 && c->Position().Rho()<minrho) {
      //if (  MathUtils::DeltaR(*p,*c)<0.3 && TMath::Max(nhb1,nhb2)<2 && c->Position().Rho()<minrho) {
