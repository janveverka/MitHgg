...
#define HGG_DEFAULT_CIC_CATEGORY_DUMPER_CONFIG src/MitHgg/Synchronization/\
python/dump_cic_categories_default_cfg.py

#define HGG_DEFAULT_CIC_CATEGORY_DUMPER_CONFIG_PATH \
  PREPEND_CMSSW_BASE_AND_STRINGIFY(HGG_DEFAULT_CIC_CATEGORY_DUMPER_CONFIG)
...
{
  const char *cfg_path = HGG_DEFAULT_CIC_CATEGORY_DUMPER_CONFIG_PATH;
  ...
  // Create the dumper:
  CiCCategoryDumper dumper(&source.chain(), cfg->getParameter<PSet>("dump"));
  // Run!
  dumper.ProduceDump();
} // main
