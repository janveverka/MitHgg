...
with open('merged_ascii_dump.txt', 'w') as ascii_output:
    ...
    ascii_output.write('# Leaf descriptor: run/I:lumi:event:%s[2]/F:' % vars_common[0])
    ascii_output.write(':'.join([name + '[2]' for name in vars_common[1:]]))
    ascii_output.write('\n')
    for ev in common:
        setA = list1[ev]
        setB = list2[ev]
