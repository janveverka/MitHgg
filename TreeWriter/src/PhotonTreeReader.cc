...
ClassImp(PhotonTreeReader)

//------------------------------------------------------------------------------
PhotonTreeReader::PhotonTreeReader(TTree* iTree) :
  fTree(iTree)
{
  Init();
}
...
//------------------------------------------------------------------------------
PhotonTreeReader::~PhotonTreeReader()
{}


//------------------------------------------------------------------------------
void PhotonTreeReader::Init()
{
...
//------------------------------------------------------------------------------
void PhotonTreeReader::InitBranchAddresses()
{
...
//------------------------------------------------------------------------------
void PhotonTreeReader::InitNameToOffsetMap()
{
  TClass *pclass = TClass::GetClass("mithep::PhotonTreeWriterPhoton<16>");
  TList  *plist  = pclass->GetListOfDataMembers();
  for (int i=0; i<plist->GetEntries(); ++i) {
...
//------------------------------------------------------------------------------
void PhotonTreeReader::SetBranchAddress(const TDataMember &member, Char_t *addr,
                                  const char *prefix)
{
  if (not (member.IsBasic() &&
           member.IsPersistent() &&
...
//------------------------------------------------------------------------------
bool PhotonTreeReader::HasValidType(const TDataMember &member)
{
  bool valid = false;
 ...
 */
Char_t* PhotonTreeReader::GetPtr(UInt_t photonIndex, const char * memberName)
{
