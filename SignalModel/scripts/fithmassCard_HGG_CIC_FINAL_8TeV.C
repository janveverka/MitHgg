...
//---------------------------------------------------------------------------------------------------------------------------
//Include the header file with the model constants
#include "modelConstants_8TeV_final.h"
...
//--------------------------------------------------------------------------------------
//Lumi reweighting
float Lumi_frac_weight_AB;
float Lumi_frac_weight_C;
float Lumi_frac_weight_D;

void setLumi_frac(TFile *file, TFile *f_data_A, TFile *f_data_B, TFile *f_data_C, TFile *f_data_D){
  TH1D*  hDEvents_data_A = (TH1D*) f_data_A->FindObjectAny("hDEvents");
  TH1D*  hDEvents_data_B = (TH1D*) f_data_B->FindObjectAny("hDEvents");
  TH1D*  hDEvents_data_C = (TH1D*) f_data_C->FindObjectAny("hDEvents");
  TH1D*  hDEvents_data_D = (TH1D*) f_data_D->FindObjectAny("hDEvents");

  TH1D*  hNPURunABObs_sig = (TH1D*) file->FindObjectAny("hNPURunABObs");
  TH1D*  hNPURunCObs_sig  = (TH1D*) file->FindObjectAny("hNPURunCObs");
  TH1D*  hNPURunDObs_sig  = (TH1D*) file->FindObjectAny("hNPURunDObs");

  float NEvents_data_AB =  hDEvents_data_A->GetSumOfWeights() + hDEvents_data_B->GetSumOfWeights();
  float NEvents_data_C = hDEvents_data_C->GetSumOfWeights();
  float NEvents_data_D = hDEvents_data_D->GetSumOfWeights();
  float NEvents_data = NEvents_data_AB + NEvents_data_C + NEvents_data_D;  
  
  float NEvents_mc_AB = hNPURunABObs_sig->GetSumOfWeights(); 
  float NEvents_mc_C = hNPURunCObs_sig->GetSumOfWeights(); 
  float NEvents_mc_D = hNPURunDObs_sig->GetSumOfWeights();
  float NEvents_mc = NEvents_mc_AB + NEvents_mc_C + NEvents_mc_D; 

  Lumi_frac_weight_AB = (NEvents_data_AB/NEvents_data)/(NEvents_mc_AB/NEvents_mc);
  Lumi_frac_weight_C = (NEvents_data_C/NEvents_data)/(NEvents_mc_C/NEvents_mc);
  Lumi_frac_weight_D = (NEvents_data_D/NEvents_data)/(NEvents_mc_D/NEvents_mc);
}

float lumiweight_rundep(int run) {
  if(run==194533){
    return Lumi_frac_weight_AB;
  }
  if(run==200519){
    return Lumi_frac_weight_C;
  }
  if(run==206859){
    return Lumi_frac_weight_D;
  }
}

//--------------------------------------------------------------------------------------
//PU reweighting run independent
TH1D* puweights;
...
//--------------------------------------------------------------------------------------
//PU reweighting run dependent

TH1D *puweights_AB;
TH1D *puweights_C;
TH1D *puweights_D;

void setpuweights_AB(TFile *file, TH1D *target) {//ming:set puweights for each process; might not be necessary when the pu distributions for different mc are the same
  TDirectory *dirmcpv = (TDirectory*)file->FindObjectAny("AnaFwkMod");
  TH1D       *hnpu  = (TH1D*)dirmcpv->Get("hNPURunABObs");
  //   TH1D       *hpumc = new TH1D("thNPU","",60,0.,60.);
  //   for( int i=0; i<=61; ++i)
  //     hpumc->SetBinContent(i,hnpu->GetBinContent(i));
  ...
  if( puweights_AB )
    delete puweights_AB;  // delete old guy, not needed anymore..
  
  puweights_AB = new TH1D((*htargettmp)/(*hpumc));
  
  delete htargettmp;
  return;
}

void setpuweights_C(TFile *file, TH1D *target) {//ming:set puweights for each process; might not be necessary when the pu distributions for different mc are the same
  TDirectory *dirmcpv = (TDirectory*)file->FindObjectAny("AnaFwkMod");
  TH1D       *hnpu  = (TH1D*)dirmcpv->Get("hNPURunCObs");
  //   TH1D       *hpumc = new TH1D("thNPU","",60,0.,60.);
  //   for( int i=0; i<=61; ++i)
  //     hpumc->SetBinContent(i,hnpu->GetBinContent(i));
  ...
  if( puweights_C )
    delete puweights_C;  // delete old guy, not needed anymore..
  
  puweights_C = new TH1D((*htargettmp)/(*hpumc));
  
  delete htargettmp;
  return;
}

void setpuweights_D(TFile *file, TH1D *target) {//ming:set puweights for each process; might not be necessary when the pu distributions for different mc are the same
  TDirectory *dirmcpv = (TDirectory*)file->FindObjectAny("AnaFwkMod");
  TH1D       *hnpu  = (TH1D*)dirmcpv->Get("hNPURunDObs");
  //   TH1D       *hpumc = new TH1D("thNPU","",60,0.,60.);
  //   for( int i=0; i<=61; ++i)
  //     hpumc->SetBinContent(i,hnpu->GetBinContent(i));
  ...
  if( puweights_D )
    delete puweights_D;  // delete old guy, not needed anymore..
  
  puweights_D = new TH1D((*htargettmp)/(*hpumc));
...
float puweight_rundep(float npu, int run) {
  if (npu<0) return 1.0;

  if(run==194533){
    return puweights_AB->GetBinContent(puweights_AB->FindFixBin(npu));
  }
  if(run==200519){
    return puweights_C->GetBinContent(puweights_C->FindFixBin(npu));
  }
  if(run==206859){
    return puweights_D->GetBinContent(puweights_D->FindFixBin(npu));
  }
}

//--------------------------------------------------------------------------------------
//VtxID Eff reweighting
TGraphAsymmErrors** vtxWeights_pass  = new TGraphAsymmErrors*[16];
TGraphAsymmErrors** vtxWeights_fail  = new TGraphAsymmErrors*[16];

float vtxWeight(float ptgg, float vtxz, float genHiggsZ, int iCat) {
  
  float diffVtx = TMath::Abs(vtxz - genHiggsZ);
  
  if (diffVtx < 0.1) {
    // this is the correct guy
    if (!vtxWeights_pass[iCat]) return 1.0;
    return vtxWeights_pass[iCat]->Eval(ptgg);
  }

  if (!vtxWeights_fail[iCat]) return 1.0;
  return vtxWeights_fail[iCat]->Eval(ptgg);
...
TTree* ApplyVBFMVAAsFriend(TTree *intree, TString tmvaweights_dijet, TString tmvaweights_combined, std::string targetname) {
  
  TMVA::Reader* fReader_dijet = new TMVA::Reader( "!Color:!Silent:Error" ); 
  TMVA::Reader* fReader_combined = new TMVA::Reader( "!Color:!Silent:Error" );       
  
  TString Weights_dijet = tmvaweights_dijet;
  TString Weights_combined = tmvaweights_combined;
  ...
  float _jet1eta, _jet2eta, _ph1pt, _ph2pt, _mass, _ptgg,_min_dphidijetgg;

  float _diphobdt,_dijetbdt;
  ...
  intree->SetBranchAddress("bdt", &_diphobdt);

  // TMVA input variables
  fReader_dijet->AddVariable("dijet_leadEta",&_jet1eta);
  fReader_dijet->AddVariable("dijet_subleadEta",&_jet2eta);
  fReader_dijet->AddVariable("dijet_LeadJPt",&_jet1pt);
  fReader_dijet->AddVariable("dijet_SubJPt",&_jet2pt);
  fReader_dijet->AddVariable("dijet_Zep",&_zeppenfeld);
  fReader_dijet->AddVariable("min(dijet_dPhi,2.916)",&_min_dphidijetgg);
  fReader_dijet->AddVariable("dijet_Mjj",&_dijetmass);
  fReader_dijet->AddVariable("dipho_pt/mass",&_diphoptOverdiphomass);  
 
  fReader_dijet->BookMVA("BDT method",Weights_dijet);

  assert(fReader_dijet);

  // TMVA input variables
  fReader_combined->AddVariable("dipho_mva",&_diphobdt);
  fReader_combined->AddVariable("bdt_dijet_maxdPhi",&_dijetbdt);
  fReader_combined->AddVariable("dipho_pt/mass",&_diphoptOverdiphomass);
   
  fReader_combined->BookMVA("BDT method",Weights_combined);

  assert(fReader_combined);
  ...
  for (Long64_t iev=0; iev<intree->GetEntries(); ++iev) {
    ...
    _min_dphidijetgg = TMath::Min(TMath::Abs(_dphidijetgg),2.916);

    if( (_pho1ptOverdiphomass > 40/120) && (_pho2ptOverdiphomass > 30/120) && (_jet1pt > 30) && (_jet2pt > 20) && (_dijetmass > 250) ) {
      _dijetbdt = fReader_dijet->EvaluateMVA("BDT method");
      target = fReader_combined->EvaluateMVA("BDT method");
    }
  ...
  //clear TMVA reader
  delete fReader_dijet;
  delete fReader_combined;
...
TTree *ApplyAsFriend(TTree *intree, TString tmvaweights, std::string targetname, bool scaleIDMVA = false, bool scaleSigEoE = false) {
  
  TMVA::Reader* tmva = new TMVA::Reader();
  
  TString Weights = tmvaweights;
  
  float _relRes, _relResWrongVtx, _vtxProb, _pt1m, _pt2m, _ph1eta, _ph2eta, _dphi, _ph1idmva, _ph2idmva;

  float _ph1pt, _ph2pt, _mass;

  float _ph1phi, _ph2phi;
  
  // input variables
  intree->SetBranchAddress("masserrsmeared", &_relRes);
  intree->SetBranchAddress("masserrsmearedwrongvtx", &_relResWrongVtx);
  intree->SetBranchAddress("vtxprob", &_vtxProb);
  intree->SetBranchAddress("ph1.pt", &_ph1pt);
  intree->SetBranchAddress("ph2.pt", &_ph2pt);
  intree->SetBranchAddress("mass", &_mass);
  intree->SetBranchAddress("ph1.eta", &_ph1eta);
  intree->SetBranchAddress("ph2.eta", &_ph2eta);
  intree->SetBranchAddress("ph1.phi", &_ph1phi);
  intree->SetBranchAddress("ph2.phi", &_ph2phi);
  intree->SetBranchAddress("ph1.idmva", &_ph1idmva);
  intree->SetBranchAddress("ph2.idmva", &_ph2idmva);
  ...
  tmva->BookMVA("BDT method",Weights);
  ...
  for (Long64_t iev=0; iev<intree->GetEntries(); ++iev) {
    ...
    target = tmva->EvaluateMVA("BDT method");
    if (iev%1000000==0) printf("%i\n",int(iev));
    friendtree->Fill();
...
TTree *ApplyAsFriendAllbdt(TTree *intree, TString tmvaweights_dipho, TString tmvaweights_dijet, TString tmvaweights_comb, std::string targetname_dipho,std::string targetname_comb) {
  
  UInt_t _evt;

  float _ph1pt, _ph2pt, _ph1phi, _ph2phi, _mass, _ptgg, _dphidijetgg;

  float _relRes, _relResWrongVtx, _vtxProb, _pt1m, _pt2m, _ph1eta, _ph2eta, _dphi, _ph1idmva, _ph2idmva;//dipho bdt

  float _jet1eta, _jet2eta, _jet1pt, _jet2pt, _zeppenfeld, _min_dphidijetgg, _dijetmass, _diphoptOverdiphomass;//dijet bdt
  
  float _diphobdt,_dijetbdt, _combbdt;

  // input variables
  intree->SetBranchAddress("evt", &_evt);
  intree->SetBranchAddress("ph1.pt", &_ph1pt);
  intree->SetBranchAddress("ph2.pt", &_ph2pt);
  intree->SetBranchAddress("ph1.phi", &_ph1phi);
  intree->SetBranchAddress("ph2.phi", &_ph2phi);
  ...
  //TMVA dipho
  TMVA::Reader* tmva_dipho = new TMVA::Reader();
  tmva_dipho->AddVariable("masserrsmeared/mass", &_relRes);
  tmva_dipho->AddVariable("masserrsmearedwrongvtx/mass", &_relResWrongVtx);
  tmva_dipho->AddVariable("vtxprob", &_vtxProb);  
  tmva_dipho->AddVariable("ph1.pt/mass", &_pt1m);
  tmva_dipho->AddVariable("ph2.pt/mass", &_pt2m);
  tmva_dipho->AddVariable("ph1.eta", &_ph1eta);
  tmva_dipho->AddVariable("ph2.eta", &_ph2eta);
  tmva_dipho->AddVariable("TMath::Cos(ph1.phi-ph2.phi)", &_dphi);    
  tmva_dipho->AddVariable("ph1.idmva", &_ph1idmva);    
  tmva_dipho->AddVariable("ph2.idmva", &_ph2idmva); 

  tmva_dipho->BookMVA("BDT method",tmvaweights_dipho);

  //TMVA dijet
  TMVA::Reader* tmva_dijet = new TMVA::Reader(); 
  tmva_dijet->AddVariable("dijet_leadEta",&_jet1eta);
  tmva_dijet->AddVariable("dijet_subleadEta",&_jet2eta);
  tmva_dijet->AddVariable("dijet_LeadJPt",&_jet1pt);
  tmva_dijet->AddVariable("dijet_SubJPt",&_jet2pt);
  tmva_dijet->AddVariable("dijet_Zep",&_zeppenfeld);
  tmva_dijet->AddVariable("min(dijet_dPhi,2.916)",&_min_dphidijetgg);
  tmva_dijet->AddVariable("dijet_Mjj",&_dijetmass);
  tmva_dijet->AddVariable("dipho_pt/mass",&_diphoptOverdiphomass);  
 
  tmva_dijet->BookMVA("BDT method",tmvaweights_dijet);

  //TMVA comb
  TMVA::Reader* tmva_comb = new TMVA::Reader();  
  tmva_comb->AddVariable("dipho_mva",&_diphobdt);
  tmva_comb->AddVariable("bdt_dijet_maxdPhi",&_dijetbdt);
  tmva_comb->AddVariable("dipho_pt/mass",&_diphoptOverdiphomass);
   
  tmva_comb->BookMVA("BDT method",tmvaweights_comb);

  //initialize new friend tree
  TTree *friendtree = new TTree();
  friendtree->SetName("allbdt");
  friendtree->Branch(targetname_dipho.c_str(),&_diphobdt,TString::Format("%s/F",targetname_dipho.c_str()));
  friendtree->Branch(targetname_comb.c_str(),&_combbdt,TString::Format("%s/F",targetname_comb.c_str()));
  ...
  for (Long64_t iev=0; iev<intree->GetEntries(); ++iev) {
    ...
    _diphobdt = tmva_dipho->EvaluateMVA("BDT method");
    _combbdt = -99;
      
    if( (_pt1m > 40/120) && (_pt2m > 30/120) && (_jet1pt > 30) && (_jet2pt > 20) && (_dijetmass > 250) ) {
      _dijetbdt = tmva_dijet->EvaluateMVA("BDT method");
      _combbdt = tmva_comb->EvaluateMVA("BDT method");
    }
...
bool readWeightsFromConfigCard(TString fileName, 
			       TH1D*& puweight_AB, TH1D*& puweight_C, TH1D*& puweight_D, TFile*& ptfile, TString& vtxWeightFileName);

bool readParmsAndCatsFromConfigCard( TString fileName,
				     bool& computeMVAvar,
				     bool& computeDIJETMVAvar,
				     bool& computeCOMBMVAvar,
				     bool& correctIDMVAvar,
				     bool& correctSigEoEvar,
				     TString& mvaWeightFile,
				     TString& DIJETmvaWeightFile,
				     TString& COMBmvaWeightFile,
				     TString& mvaDefFile,
...
bool readScaleSmearNuissances(TString configCardName,
                              std::vector<RooRealVar*>& scaleSmearNuissances,
                              std::vector<std::vector<std::vector<double> > > &scalevals,
                              std::vector<std::vector<std::vector<double> > > &smearvals);
...
RooDataSet *makedset(TString name, TTree *tree, TCut cut, RooRealVar *hmass, RooRealVar *weight, double nomMass, bool& empty, double moreWeights = 1., TH1D* hist = NULL) {
  ...
  if (nev > 0 && nev <=2){
    for (int iev=0; iev<nev; ++iev) {
      sumweights = sumweights + weights[iev]*moreWeights;
    }
    vset->setVal(nomMass);
    dset->add(*vset,sumweights);
    vset->setVal(nomMass-2.);
    dset->add(*vset,sumweights/2.);
    vset->setVal(nomMass+2.);
    dset->add(*vset,sumweights/2.);
  }
...
// ============================================================================================
void fithmassCard_HGG_CIC_FINAL_8TeV(bool fitonly        = false, 
				     bool doeffsigma     = true,
				     bool doeffsmear     = false,
				     bool doSecondSignal = false,
				     TString baseString  = "2013final_8TeV",
				     //TString configCardName="/home/fabstoec/cms/root/templateHGG_8TeV_CiC_Moriond.config") {
				     //TString configCardName="templateHGG_8TeV_MVA_FINAL.config"
                                     //TString configCardName="/home/bendavid/cms/cmssw/031/CMSSW_5_3_10_patch1/src/sigscripts/templateHGG_8TeV_MVA_FINAL.config") {
				     TString configCardName="templateHGG_8TeV_CIC_FINAL.config") {
  ...
  if( !hpuest_AB )
    std::cerr<<" [INFO] PU-rewighting AB switched OFF."<<std::endl;

  if( !hpuest_C )
    std::cerr<<" [INFO] PU-rewighting C switched OFF."<<std::endl;

  if( !hpuest_D )
    std::cerr<<" [INFO] PU-rewighting D switched OFF."<<std::endl;
  ...
  if ( vtxWeightFileName.CompareTo("") ) {
    ...
    for(int iCat = 0; iCat < 16; ++iCat){
      TString graphName_pass = TString::Format("ratioVertex_cat%d_pass", iCat).Data();
      TString graphName_fail = TString::Format("ratioVertex_cat%d_fail", iCat).Data();
      
      vtxWeights_pass[iCat] = (TGraphAsymmErrors*) vtxWeightFile->Get(graphName_pass.Data());
      vtxWeights_fail[iCat] = (TGraphAsymmErrors*) vtxWeightFile->Get(graphName_fail.Data());
      
      if (!vtxWeights_pass[iCat] || !vtxWeights_fail[iCat]) {
  ...
  readScaleSmearNuissances("/home/mingyang/cms/cmssw/031/CMSSW_5_3_10_patch1/src/MitHgg/macros/sigscripts_fromJosh/scaleSmear_CIC_8TeV.dat",
                            scaleSmearNuissances,
                            scalevals,
                            smearvals);  
...
//     pSS <<  TString(procxseclist_names->at(iComp));
  ...
  TString weights_dijet   = "";
  TString weights_combined   = "";
  ...
  if( computeDIJETMVAvar ) {
    weights_dijet = DIJETmvaWeightFile;
  }

  if( computeCOMBMVAvar ) {
    weights_combined = COMBmvaWeightFile;
  }
  ...
  for(int iProc = 0; iProc < numProcs; ++iProc) {
	...
	treeNS = (TTree*)nsdir->FindObjectAny(nosmeartreename.Data());  // FIX-ME
      ...
      /*if( computeMVAvar ) {
	ApplyAsFriend(tree,weights,"bdt",correctIDMVAvar,correctSigEoEvar);
	ApplyVBFMVAAsFriend(tree,weights_dijet,weights_combined,"bdt_combined");
 	if(doeffsmear){
	  ApplyAsFriend(treeNS,weights,"bdt",correctIDMVAvar,correctSigEoEvar);
	  ApplyVBFMVAAsFriend(treeNS,weights_dijet,weights_combined,"bdt_combined");
	}
	}*/

      if( computeMVAvar ) {
	ApplyAsFriendAllbdt(tree,weights,weights_dijet,weights_combined,"bdt","bdt_combined");
	if(doeffsmear){
	  ApplyAsFriendAllbdt(treeNS,weights,weights_dijet,weights_combined,"bdt","bdt_combined");
	}
      }
...
//     allCan->Divide(numMasses, numCats);
	...
	if( hpuest_AB )
	  setpuweights_AB(files[i], hpuest_AB);

	if( hpuest_C )
	  setpuweights_C(files[i], hpuest_C);

	if( hpuest_D )
	  setpuweights_D(files[i], hpuest_D);

	setLumi_frac(files[i], datafiles[0], datafiles[1], datafiles[2], datafiles[3]);
	...
	if( catIt == anaCatMap.end() ) {
	  ...
	  return;
        ...
        //TCut theWeight = "puweight_rundep(numPU,run)*lumiweight_rundep(run)*bsweight(vtxZ,genHiggsZ)*trigeffscale()*effweightCard(ph1.sceta,ph1.r9)*effweightCard(ph2.sceta,ph2.r9)*vtxWeight(ptgg,vtxZ,genHiggsZ)";
	TCut theWeight = TString::Format("puweight_rundep(numPU,run)*lumiweight_rundep(run)*vtxWeight(ptgg,vtxZ,genHiggsZ,%d)*bsweight(vtxZ,genHiggsZ)*trigeffscale()*effweightCard(ph1.sceta,ph1.r9)*effweightCard(ph2.sceta,ph2.r9)",iCat).Data();
	...
	if ( mhs_fit[i] == 0 ) {
	  ...
	  printf("nevtnoweight=%f  eacc = %5f, eaccnum = %5f, eaccden = %5f\n",eaccnumModel,eacc,eaccnumModel,eaccden);
	  ...
	  printf("nevt=%f eacc = %5f, eaccnum = %5f, eaccden = %5f\n",eaccnumModel, eacc,eaccnumModel,eaccden);
	...
	// append this DataSet to the per-mass/per-cat datasetss
   ...
   std::vector<RooAbsReal*> finalnormslidesModel;
    ...
    for(int iCat=0; iCat < numCats; ++iCat) {
      ...
      std::vector<double> &pcscalevals = scalevals[iCat][iProc];
      std::vector<double> &pcsmearvals = smearvals[iCat][iProc];
      
      for (int inuis = 0; inuis<pcscalevals.size(); ++inuis) {
        double scaleval = pcscalevals[inuis];
        double smearval = pcsmearvals[inuis];
        
        if (scaleval>=1e-4) {
          RooProduct *meanprod = new RooProduct(TString::Format("meanprod_%i",inuis)+TString("_") + procname +TString("_")+catnames[iCat],"",RooArgList(*scaleSmearNuissances[inuis],RooConst(scaleval)));
          scaleadd.add(*meanprod);
        }
        if (smearval>=1e-4) {
          RooProduct *sigmaprod = new RooProduct(TString::Format("sigmaprod_%i",inuis)+TString("_") + procname +TString("_")+catnames[iCat],"",RooArgList(*scaleSmearNuissances[inuis],RooConst(sqrt(pow(1.0+smearval,2) - 1.0))));
          smearadd.add(*sigmaprod);
        }
      }
      RooAddition *scalesum = new RooAddition(TString("scalesum")+TString("_") + procname +TString("_")+catnames[iCat],"",scaleadd);
      RooAddition *smearsum = new RooAddition(TString("smearsum")+TString("_") + procname +TString("_")+catnames[iCat],"",smearadd);
	...
	while ( gIt != fitparms[iCat].end() ) {
	  ...
	  if ( iVtx )
	    meanslidesG_wv[iCat][gCounter-1] = new RooFormulaVar(slideName.Data(),"","@0 + @1 +@0*@2",RooArgList(mnom,*(pair->second),*scalesum));
	  else
	    meanslidesG_rv[iCat][gCounter-1] = new RooFormulaVar(slideName.Data(),"","@0 + @1 +@0*@2",RooArgList(mnom,*(pair->second),*scalesum));	  
	  ...
	  if ( iVtx ) 
	    sigmaslidesG_wv[iCat][gCounter-1] = new RooFormulaVar(slideName.Data(),"","@0*TMath::Max(0.01,sqrt(1.0+@1))",RooArgList(*(pair->second),*smearsum));//why set min 0.01?	
	  else
	    sigmaslidesG_rv[iCat][gCounter-1] = new RooFormulaVar(slideName.Data(),"","@0*TMath::Max(0.01,sqrt(1.0+@1))",RooArgList(*(pair->second),*smearsum));//why set min 0.01?	
...
bool readScaleSmearNuissances(TString configCardName,
  ...
  std::map<std::string, int> nuismap;
  ...
  while (fgets(line,100000,configFile)) {
    // must be some nuissance or normalization stuff...
    char procname[30];
    char systname[50];
    ...
    if( sscanf(line,"diphotonCat %d",&icat) ) { 
      printf("icat = %i\n",icat);
      ;
    }
    else if( sscanf(line,"proc %s",procname) ) { 
      if (std::string(procname)=="ggh") iproc = 0;
      else if (std::string(procname)=="vbf") iproc = 1;
      else if (std::string(procname)=="wh") iproc = 2;
      else if (std::string(procname)=="tth") iproc = 3;
      else if (std::string(procname)=="zh") iproc = -1;
      
      printf("iproc = %i\n",iproc);
    }  
    else if( sscanf(line,"%s %f %f %f",systname,&meanshift,&sigmashift,&yieldshift) ) { 
      printf("matched line, icat = %i, iproc = %i\n",icat,iproc);
      ...
      std::map<std::string, int>::const_iterator systit = nuismap.find(std::string(systname));
      if (systit == nuismap.end()) {
        RooRealVar *nuis = new RooRealVar(systname, "",0.,-5.,5.);
        nuis->setConstant();
        scaleSmearNuissances.push_back(nuis);
        nuisidx = scaleSmearNuissances.size()-1;
        nuismap.insert(std::make_pair(std::string(systname),nuisidx));
        printf("added nuisance %i\n",nuisidx);
      }
      else {
        nuisidx = systit->second;
        printf("found nuisance %i\n",nuisidx);
      }
      
      if (scalevals.size()<=icat) scalevals.resize(icat+1);
      if (smearvals.size()<=icat) smearvals.resize(icat+1);

      if (scalevals[icat].size()<=iproc) scalevals[icat].resize(iproc+1);
      if (smearvals[icat].size()<=iproc) smearvals[icat].resize(iproc+1);
      
      std::vector<double> &pcscalevals = scalevals[icat][iproc];
      std::vector<double> &pcsmearvals = smearvals[icat][iproc];
      
      if (pcscalevals.size()<=nuisidx) pcscalevals.resize(nuisidx+1);
      if (pcsmearvals.size()<=nuisidx) pcsmearvals.resize(nuisidx+1);
      
      pcscalevals[nuisidx] = meanshift;
      pcsmearvals[nuisidx] = sigmashift;
      
    } 
  }
  
  for (int icat=0; icat<scalevals.size(); ++icat) {
    for (int iproc=0; iproc<scalevals[icat].size(); ++iproc) {
      for (int inuis=0; inuis < scalevals[icat][iproc].size(); ++inuis) {
        printf("icat = %i, iproc = %i, inuis = %i, scaleval = %5e, smearval = %5e\n",icat,iproc,inuis,scalevals[icat][iproc][inuis],smearvals[icat][iproc][inuis]);
      }
    }
  }
...
bool readParmsAndCatsFromConfigCard( TString fileName,
  ...
  while (fgets(line,1000,configFile)) {
    ...
    else if  ( sscanf(line,"COMPUTEDIJETMVA ON %s", name) ) { DIJETmvaWeightFile = TString(name); computeDIJETMVAvar = true; }
    else if  ( sscanf(line,"COMPUTECOMBMVA ON %s", name) ) { COMBmvaWeightFile = TString(name); computeCOMBMVAvar = true; }
...
bool readWeightsFromConfigCard(TString fileName, 
  ...
  while (fgets(line,1000,configFile)) {
    ...
    if( sscanf(line,"PUREWEIGHFILE_AB ON %s", weightFileName) ) {
      //   //Load pileup weights
      TFile *filepuest_AB = new TFile(weightFileName);
      if( !filepuest_AB ) {
	std::cerr<<" ERROR: Could not open PU file with name "<<weightFileName<<"."<<std::endl;
	fclose(configFile);
	return false;
      }
      TH1D *hpuest_AB = (TH1D*) filepuest_AB->Get("pileup");
      if( !hpuest_AB ) {
	...
	return false;
      ...
      puweight_AB = hpuest_AB;

    } else if( sscanf(line,"PUREWEIGHFILE_C ON %s", weightFileName) ) {
      //   //Load pileup weights
      TFile *filepuest_C = new TFile(weightFileName);
      if( !filepuest_C ) {
	std::cerr<<" ERROR: Could not open PU file with name "<<weightFileName<<"."<<std::endl;
	fclose(configFile);
	return false;
      }
      TH1D *hpuest_C = (TH1D*) filepuest_C->Get("pileup");
      if( !hpuest_C ) {
	std::cerr<<" ERROR: Could not read histogram <pileup> from PU file with name "<<weightFileName<<"."<<std::endl;
	fclose(configFile);
	return false;
    ...
    } else if( sscanf(line,"PUREWEIGHFILE_D ON %s", weightFileName) ) {
      //   //Load pileup weights
      TFile *filepuest_D = new TFile(weightFileName);
      if( !filepuest_D ) {
	std::cerr<<" ERROR: Could not open PU file with name "<<weightFileName<<"."<<std::endl;
	fclose(configFile);
	return false;
      }
      TH1D *hpuest_D = (TH1D*) filepuest_D->Get("pileup");
      if( !hpuest_D ) {
	std::cerr<<" ERROR: Could not read histogram <pileup> from PU file with name "<<weightFileName<<"."<<std::endl;
	fclose(configFile);
	return false;
