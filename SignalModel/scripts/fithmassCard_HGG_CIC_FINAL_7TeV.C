...
#include "modelConstants_7TeV_final.h"
...
float puweight_rundep(float npu) {
  if (npu<0) return 1.0;
...
TGraphAsymmErrors** vtxWeights_pass  = new TGraphAsymmErrors*[15];
TGraphAsymmErrors** vtxWeights_fail  = new TGraphAsymmErrors*[15];
...
TH1D** ptweights = new TH1D*[9];
double ptweight(double genhpt, Int_t procid, Int_t mh) {
  int imh = (mh-110)/5;
  if (procid>0) return 1.0;
  if (genhpt<0) return 1.0;
  return ptweights[imh]->GetBinContent(ptweights[imh]->FindFixBin(genhpt));
} 

//--------------------------------------------------------------------------------------
float trigeffscale() {  

  return (float) 1.;//check
...
TTree* ApplyVBFMVAAsFriend(TTree *intree, TString tmvaweights_dijet, TString tmvaweights_combined, std::string targetname) {
  ...
  fReader_dijet->AddVariable("min(dijet_dPhi,2.9416)",&_min_dphidijetgg);
  ...
  fReader_combined->AddVariable("bdt_dijet_7TeV_ptrewght",&_dijetbdt);
  ...
  for (Long64_t iev=0; iev<intree->GetEntries(); ++iev) {
    ...
    _min_dphidijetgg = TMath::Min(TMath::Abs(_dphidijetgg),2.9416);

    if( (_jet1pt > 30) && (_jet2pt > 20) && (_dijetmass > 250) ) {
...
TTree *ApplyAsFriendAllbdt(TTree *intree, TString tmvaweights_dipho, TString tmvaweights_dijet, TString tmvaweights_comb, std::string targetname_dipho,std::string targetname_comb) {
  ...
  tmva_dijet->AddVariable("min(dijet_dPhi,2.9416)",&_min_dphidijetgg);
  ...
  tmva_comb->AddVariable("bdt_dijet_7TeV_ptrewght",&_dijetbdt);
...
// ============================================================================================
void fithmassCard_HGG_CIC_FINAL_7TeV(bool fitonly        = false, 
				     bool doeffsigma     = true,
				     bool doeffsmear     = false,
				     bool doSecondSignal = false,
				     TString baseString  = "2013final_7TeV",
				     //TString configCardName="/home/bendavid/cms/cmssw/031/CMSSW_5_3_10_patch1/src/sigscripts/templateHGG_7TeV_MVA_FINAL.config") {
				     TString configCardName="templateHGG_7TeV_CIC_FINAL.config") {
  ...
  TH1D*  hpuest_C       = NULL;
  TH1D*  hpuest_D       = NULL;
  TFile *fileptweight = new TFile("/home/mingyang/cms/2013Final/FINAL/7TeV/input/ptweights/KFactors_AllScales.root","READ");
  ...
  // --------------------------------------------------------------------------------------------------
  // set up the pt weights
  for(int imh=0; imh<9; imh++){
    int mh = 110+imh*5;
    ptweights[imh]= (TH1D*) fileptweight->Get(TString::Format("kfact%d_0", mh).Data());
  }
  ...
  readScaleSmearNuissances("/home/mingyang/cms/cmssw/031/CMSSW_5_3_10_patch1/src/MitHgg/macros/sigscripts_fromJosh/scaleSmear_CIC_7TeV.dat",
...
//     allCan->Divide(numMasses, numCats);
	...
	if( hpuest_AB )
	  setpuweights(files[i], hpuest_AB);
	...
	if( catIt == anaCatMap.end() ) {
	  ...
	  return;
      	...
      	TCut theWeight = TString::Format("ptweight(genHiggspt, %d, %d)*puweight(numPU)*vtxWeight(ptgg,vtxZ,genHiggsZ,%d)*trigeffscale()*effweightCard(ph1.sceta,ph1.r9)*effweightCard(ph2.sceta,ph2.r9)",iProc,int(mhs[i]),iCat).Data();
   ...
   std::vector<RooAbsReal*> finalnormslidesModel;
    ...
    RooRealVar* globalScale = new RooRealVar("CMS_hgg_globalscale_7TeV","",0.0,-0.004717,0.004717);
