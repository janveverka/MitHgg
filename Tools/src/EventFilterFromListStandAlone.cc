...
ClassImp(EventFilterFromListStandAlone)
...
EventFilterFromListStandAlone::EventFilterFromListStandAlone(const std::string & eventFileName)
{
  ...
  if (verbose_) cerr << "Event list from file "<<eventFileName<<endl;
  readEventListFile(eventFileName);
  std::sort(EventList_.begin(), EventList_.end());
  if (verbose_) cerr<<" A total of "<<EventList_.size()<<" listed events found in given run range"<<endl;
...
void EventFilterFromListStandAlone::addEventString(const string & eventString)
{
  ...
  if (found!=std::string::npos)
    ...
    {
      cerr<<"  Unable to parse Event list input '"<<eventString<<"' for run number!\n";
      return;
    }
  ...
  if (found2!=std::string::npos)
    {
      ...
      if (ls==0 || event==0) cerr<<"  Strange lumi, event numbers for input '"<<eventString<<"'";
    }
  else
    {
      cerr<<"Unable to parse Event list input '"<<eventString<<"' for run number!\n";
  ...
  if (minrun_>-1 && run<minrun_)
    {
      if (verbose_)  cerr <<"Skipping Event list input '"<<eventString<<"' because it is less than minimum run # "<<minrun_<<endl;
      return;
    }
  if (maxrun_>-1 && run>maxrun_)
    {
      if (verbose_) cerr <<"Skipping Event list input '"<<eventString<<"' because it is greater than maximum run # "<<maxrun_<<endl;
      return;
    }
...
void EventFilterFromListStandAlone::readEventListFile(const string & eventFileName)
{
  ...
  if (! file) {
    cerr << "  Unable to open event list file `" << eventFileName
         << "'" << endl;
    return;
  }
  string b2;
  ...
  while (1) {
    ...
    if (bytes_read < LENGTH - 1) {
      if (gzeof (file)) break;
          ...
          if (err) {
            cerr<<"Error while reading gzipped file:  "<<error_string<<endl;
            return;
          }
        }
    }
...
EventFilterFromListStandAlone::filter(int run, int lumiSection, int event)
{
  ...
  // Dump out via cerr, or via LogInfo?  For now, use cerr
  if (verbose_) std::cerr <<"EventFilterFromListStandAlone removed "<<thisevent.str()<<std::endl;
